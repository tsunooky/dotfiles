...
finalize() {
    ...
    # dunstrc, flameshot.ini, lock.sh and every colors.* file are generated by
    # matugen from ~/.config/matugen/templates: render them now from the current
    # wallpaper so template changes apply immediately. On a fresh install this
    # is done by autostart_once.sh at the first i3 start.
    if [ -f "$HOME/.curbg" ] && command -v matugen >/dev/null 2>&1; then
        log "${CYAN}• Rendering matugen templates from the current wallpaper...${NC}"
        if ~/.config/scripts/matugen-reload.sh >> "${LOGFILE}" 2>&1; then
            log "${GREEN}✓ Templates rendered${NC}"
        else
            log "${YELLOW}⚠ matugen failed, run 'bg <image>' after reboot (see ${LOGFILE})${NC}"
        fi
    fi
